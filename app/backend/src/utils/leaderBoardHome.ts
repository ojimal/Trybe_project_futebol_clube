...
export default class LeaderBoard {
  ...
  private setVictory(homeTeamGoals: number, awayTeamGoals: number) {
    ...
    this.goalsFavor = goalsFavor + homeTeamGoals;
    this.goalsOwn = goalsOwn + awayTeamGoals;
  ...
  public returnPointsHome(MatchI: MatchI[]) {
    // Calculate total points for home matches
    MatchI.forEach(({ homeTeamGoals, awayTeamGoals }) => {
      if (homeTeamGoals > awayTeamGoals) this.setVictory(homeTeamGoals, awayTeamGoals);
      if (homeTeamGoals === awayTeamGoals) this.setDraw(homeTeamGoals, awayTeamGoals);
      if (homeTeamGoals < awayTeamGoals) this.setLoss(homeTeamGoals, awayTeamGoals);
    });
    return {
      name: this.name,
